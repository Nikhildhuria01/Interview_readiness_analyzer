...
def analyze_single_answer(answer):
    ...
    transcript = generate_transcript(answer["answer_file"], answer["transcript_file"])
    ...
    ideal_answer = generate_ideal_answer(answer["question"])
    ...
    correctness_result = calculate_correctness(
        answer["question"], transcript, ideal_answer
    )
...
def analyze_all_answers(recorded_answers):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        futures = [
            executor.submit(analyze_single_answer, answer)
...
def run_audio_interview():
    ...
    save_training_data(
        avg_fluency, avg_correctness, eye_score, post_score, head_score, overall
    )
